...
모든 소스 코드, 스크립트, 설정 파일, 검증 결과는 반드시 이 저장소 안에 작성한다.

- 프로젝트 루트: `/home/qsentech/hl-dcu-sensor-fusion`
- 임시 디렉토리(`~/halla_sensor_ws`, `/tmp` 등)에 작업 파일을 생성하지 않는다.
- 검증 목적의 스크립트도 `src/<패키지>/scripts/` 에 바로 작성한다.

## 3. Session Start

새 대화를 시작할 때 반드시 아래 순서로 컨텍스트를 파악한다.

1. 프로젝트 루트의 모든 `*.md` 파일을 읽는다.
2. `docs/` 하위의 `*.md` 파일을 읽는다.
3. `PROJECT_STATUS.md`로 현재 진행 상태를 파악한다.
4. 파악한 내용을 바탕으로 사용자 요청을 해석한다.
...
상태 기록에는 반드시 날짜(`YYYY-MM-DD`)를 명시한다. 날짜 없는 상태 기록은
작성하지 않는다.
